...
class BenefitsMapService {
  ...
  private async loadBenefitsData() {
    try {
      ...
      benefitsData.forEach(benefit => {
        ...
        if (!this.stateStats[state]) this.stateStats[state] = { count: 0, federalCount: 0, stateCount: 0 };
  ...
  public getStateElevation = (fipsCode: string | null) => {
    ...
    return stats.count > 0 ? stats.count * 15000 : 2000; // Give all states a base elevation
  }

  // All states will now have the same initial color
  public getStateColor = () => {
    return [25, 40, 60, 200]; // Uniform futuristic dark blue
  }
}
