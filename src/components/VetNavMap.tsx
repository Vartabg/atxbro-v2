...
const VetNavMap = ({ topoJsonPath = '/data/states-albers-10m.json', targetDisplayWidth = 10, showLabels = false }) => {
        ...
        fetch(topoJsonPath)
            ...
            .then(usTopoData => {
                ...
                if (allCoords.length === 0) {
                    setTransformParams({ scale: 0.01, centerX: 0, centerY: 0 });
                ...
                setTransformParams({ scale: calculatedScale, centerX: dataCenterX, centerY: dataCenterY });
                setProcessedFeatures(geoJsonFeatures);
                setLoading(false);
            })
            .catch(err => {
    ...
    if (loading) return <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%' }}><p>Loading Map Data...</p></div>;
    if (error) return <div style={{ color: 'red', padding: '20px' }}><p>Error loading map: {error}</p></div>;
    if (!transformParams || processedFeatures.length === 0) return <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%' }}><p>No map data to display.</p></div>;
